...
export class UserService {
  ...
  findAll() {
    return this.userRepository.find({
      relations: ['playlists', 'songs']}) // This will load both relations;
  }
  ...
  async update(id: number, updateUserDto: UpdateUserDto) {
    ...
    // Add songs
    if(updateUserDto.songToAdd) {
      if (updateUserDto.songToAdd?.length > 0) {
    ...
    // Remove songs
    if (updateUserDto.songToRemove) {
      if (updateUserDto.songToRemove?.length > 0) {
    ...
    if (updateUserDto.playlistToRemove) {
      if (updateUserDto.playlistToRemove?.length > 0) {
